...
// Keep the original public interface working (not performance-critical).
std::unordered_map<uint32_t, std::vector<int>> buildHashTable(const std::vector<int>& seq,
                                                               int kmerSize, int bitshift,
                                                               uint32_t wmask) {
    std::unordered_map<uint32_t, std::vector<int>> ht;
    if (seq.size() < static_cast<size_t>(kmerSize)) return ht;
    uint32_t h = computeHash(seq, 0, kmerSize, bitshift, wmask);
    ht[h].push_back(0);
    for (int i = 1; i <= static_cast<int>(seq.size()) - kmerSize; i++) {
        h = updateHashValue(h, seq[i + kmerSize - 1], bitshift, wmask);
        ht[h].push_back(i);
    }
    return ht;
}

float hspScore(const std::string& query, const std::string& target, int offset) {
    int len1 = query.size();
    int len2 = target.size();
    int min_len = std::min(len1, len2);
    float maxsofar = 0.0f;
    float maxendinghere = 0.0f;
    for (int i = 0; i < min_len; i++) {
        int idx1 = (i < offset || i - offset >= len1) ? -1 : query[i - offset] - 'A';
        int idx2 = (i >= offset && i < len2) ? target[i] - 'A' : -1;
        float pair_score = (idx1 >= 0 && idx2 >= 0) ? substitutionMatrix[idx1][idx2] : -1.0f;
        maxendinghere = std::max(0.0f, maxendinghere + pair_score);
        maxsofar = std::max(maxsofar, maxendinghere);
    }
    return maxsofar / static_cast<float>(min_len);
}

float computeFastpScoreInternal(const std::vector<int>& qIndexed,
                                const std::vector<int>& tIndexed,
                                const Sequence& query,
                                const Sequence& target,
                                int kmerSize,
                                int bitshift,
                                uint32_t wmask,
                                const std::unordered_map<uint32_t, std::vector<int>>& targetHash,
                                const std::vector<float>& scoreFlat,
                                std::string* alignedSegment,
                                float lowerThreshold,
                                float lengthRatio) {
    const std::string& q = query.sequence;
    const std::string& t = target.sequence;

    if (q.size() < static_cast<size_t>(kmerSize) || t.size() < static_cast<size_t>(kmerSize)) {
        if (alignedSegment) *alignedSegment = "";
        return 0.0f;
    }

    if (!isLengthRatioAcceptable(query, target, lengthRatio)) {
        if (alignedSegment) *alignedSegment = "";
        return 0.0f;
    }

    const int defaultDiagOffset = 1500;
    const int defaultDiagSize = 2 * defaultDiagOffset + 1;
    const int thresholdLen = 1500;
    int diagSize = defaultDiagSize;
    static thread_local std::vector<int> diagVotes(defaultDiagSize, 0);
    int diagOffset = defaultDiagOffset;

    if (q.size() > thresholdLen || t.size() > thresholdLen) {
        int maxQueryKmers = static_cast<int>(q.size()) - kmerSize + 1;
        int maxTargetKmers = static_cast<int>(t.size()) - kmerSize + 1;
        diagOffset = maxQueryKmers;
        diagSize = maxQueryKmers + maxTargetKmers + 1;
    ...
    for (size_t i = 0; i <= q.size() - kmerSize; i++) {
        uint32_t h = computeHash(qIndexed, i, kmerSize, bitshift, wmask);
        auto it = targetHash.find(h);
        if (it != targetHash.end()) {
            seedFound = true;
            for (int pos : it->second) {
                int diag = pos - static_cast<int>(i) + diagOffset;
    ...
    float score = aliscore(qIndexed, tIndexed, best.bestDiag, bitshift, scoreFlat);
    ...
    if (alignedSegment) {
        int q_offset = best.bestDiag < 0 ? -best.bestDiag : 0;
        int t_offset = best.bestDiag > 0 ? best.bestDiag : 0;
        int align_len = std::min(static_cast<int>(q.size()) - q_offset,
                                 static_cast<int>(t.size()) - t_offset);
        *alignedSegment = t.substr(t_offset, align_len);
    }
...
float computeFastpScore(const Sequence& query,
    ...
    std::vector<int> qIndexed = indexSequence(query.sequence, charToIdx);
    std::vector<int> tIndexed = indexSequence(target.sequence, charToIdx);
    auto targetHash = buildHashTable(tIndexed, config.kmerSize, bitshift, wmask);
    const auto scoreFlat = buildScoreFlat(aminoAcids, bitshift);

    return computeFastpScoreInternal(qIndexed, tIndexed, query, target,
                                     config.kmerSize, bitshift, wmask, targetHash,
                                     scoreFlat, alignedSegment, config.lowerThreshold,
                                     config.lengthRatio);
}
